...
/*   Updated: 2021/05/08 16:04:13 by hyojang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    init_cfinfo(t_info *info, t_cfinfo *cf, int y)
{
    ...
    (cf->floorstep).x = cf->row_distance * ((cf->raydir1).x - (cf->raydir0).x) / (info->r).width;
    (cf->floorstep).y = cf->row_distance * ((cf->raydir1).y - (cf->raydir0).y) / (info->r).width;
    ...
    (cf->t).x = (int)(TEX_WIDTH * ((cf->floor).x - (cf->cell).x)) & (TEX_WIDTH - 1);
    (cf->t).y = (int)(TEX_HEIGHT * ((cf->floor).y - (cf->cell).y)) & (TEX_HEIGHT - 1);
...
void    cf_ray(t_info *info)
{
    ...
    while (y < (info->r).height)
    {
        ...
        {
            x++;
            color = info->texture[cfinfo.floortexture][TEX_WIDTH * (cfinfo.t).y + (cfinfo.t).x];
			color = (color >> 1) & 8355711;
			(info->buf)[y][x] = color; 
			color = info->texture[cfinfo.ceilingtexture][TEX_WIDTH * (cfinfo.t).y + (cfinfo.t).x];
			color = (color >> 1) & 8355711;
